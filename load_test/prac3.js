...
const TEST_DURATION_IN_MS = 1000 * 100;
...
const createClient = async(i) => {
    try {
        ...
        if (Math.random() < 0.5) {
            ...
            var { field, userInfo, userStatus } = await selector.sign[0](username)
        } else {
            const username = `test${SET_TIME}`
            console.log('UP', i, username);
            var { field, userInfo, userStatus } = await selector.sign[1](username);
        } 
        ...
        while (SET_TIME + TEST_DURATION_IN_MS > Date.now()) {
            ...
            const res2 = await battle.autoFromList(field, userInfo, userStatus, 30);
            ...
            if (field === 'heal') {
                ...
                console.log('scenario success(player dead)', res4);
    
                continue;
            }
            const res3 = await front.toHome(field, userInfo, userStatus)
            console.log('scenario success', res3);
