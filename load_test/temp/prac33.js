...
const CLIENT_CREATE_INTERVAL_IN_MS = 100;
const TEST_DURATION_IN_MS = 1000 * 180;
...
const createClient = async(i) => {
    ...
    try {
        ...
        LOOP_CHECK.add(characterId);

        // console.log('SIGNED', i, emitCount, througputs.length);
        ...
        // console.log('to dungeon', i, emitCount, througputs.length);

        const duration = (
            Math.random()*(TEST_DURATION_IN_MS/2000) + (TEST_DURATION_IN_MS/2000)
        )|0;
        ...
        LOOP_CHECK.delete(characterId);
        // console.log('battle over', i, emitCount, througputs.length);
        ...
        if (field === 'heal') {
            ...
            // console.log('healed', i, emitCount, througputs.length);

            const res4 = await front.toHome(field, userInfo, userStatus)
            emitCount += res4.cnt;
            througputs.push(...res4.throughput);
            // console.log('scenario success(player dead)', i, emitCount, througputs.length);
        ...
        emitCount += res3.cnt;
        througputs.push(...res3.throughput);
        // console.log('scenario success', i, emitCount, througputs.length);
