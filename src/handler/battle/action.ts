...
class BattleAction {
    attack = async (CMD: string | undefined, user: UserSession) => {
        ...
        const autoAttackId = setInterval(async () => {
            ...
            const {
                ...
                user: newUser,
                error,
            } = await battle.autoAttack(CMD, user);
            if (error) return;
    ...
    actionSkill = async (
        ...
        // 스킬 정보 가져오기
        const { attack, mp, skill } = await CharacterService.findByPk(
            characterId,
        );
        ...
        // 스킬 데미지 계산
        const playerSkillDamage: number = Math.floor((attack * multiple) / 100);
        const realDamage: number = BattleService.hitStrength(playerSkillDamage);

        // 스킬 Cost 적용
        user = await CharacterService.refreshStatus(
            characterId,
            0,
            cost,
            monsterId,
        ...
        // 몬스터에게 스킬 데미지 적용
        const isDead = await MonsterService.refreshStatus(
            monsterId,
            realDamage,
            characterId,
        );
