...
exports.default = {
    ...
    autoAttack: (CMD, userCache) => __awaiter(void 0, void 0, void 0, function* () {
        let tempScript = '';
        let field = 'action';
        ...
        if (isDead === 'dead') {
            ...
            const { script, field, userCache } = yield __1.battle.resultMonsterDead(monster, tempScript);
        ...
        userCache = yield services_1.CharacterService.refreshStatus(characterId, monsterHit, 0, monsterId);
    ...
    resultMonsterDead: (monster, script) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const userCache = yield services_1.CharacterService.addExp(characterId, monsterExp);
    ...
    resultPlayerDead: (userCache, script) => __awaiter(void 0, void 0, void 0, function* () {
        const { script: newScript, field, userCache: newUser, chat } = yield __1.dungeon.getDungeonList('', userCache);
        ...
        cache_1.battleCache.delete(userCache.characterId);
        return;
    }),
    autoBattle: (CMD, userCache) => __awaiter(void 0, void 0, void 0, function* () {
        let tempScript = '';
        ...
        socket_routes_1.socket.emit('printBattle', { script: monsterCreatedScript, field, userCache });
        // 자동공격 사이클
        const autoAttackTimer = setInterval(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const { script, userCache: newUser, error } = yield __1.battle.autoAttack(CMD, userCache);
            // 이미 끝난 전투
            if (error)
                return;
            // 자동공격 스크립트 계속 출력?
            const field = 'autoBattle';
            socket_routes_1.socket.emit('printBattle', { script, field, userCache: newUser });
            const whoIsDead = {
            ...
            if (dead) {
                ...
                const { script, field, userCache } = yield whoIsDead[dead]('', newUser);
                socket_routes_1.socket.emit('printBattle', { script, field, userCache });
                return;
            }
            else {
                // 스킬공격 사이클. 50% 확률로 발생
                ...
                if (chance < 0.5)
                    return;
                const { script, userCache, field } = yield __1.battle.autoBattleSkill(newUser);
                const { dead } = cache_1.battleCache.get(characterId);
                // const { dead } = await redis.hGetAll(characterId);
                socket_routes_1.socket.emit('printBattle', { script, field, userCache });
                if (dead) {
    ...
    autoBattleW: (CMD, userCache) => __awaiter(void 0, void 0, void 0, function* () {
        const { characterId } = userCache;
        ...
        socket_routes_1.socket.emit('printBattle', { script: monsterCreatedScript, field: 'autoBattle', userCache });
        ...
        // 사망판정 워커 할당 >> 소켓 송신
        isMonsterDead_1.default.check(userCache, receiver).then(({ status, script }) => {
        ...
        // 자동공격 워커 할당
        autoAttack_1.default.start(userCache, autoToDead).then((result) => {
            console.log('battle.handler.ts: 자동 공격 resolved', result, characterId);
        }).catch(error => console.error(error));
        // 스킬공격 워커 할당
        skillAttack_1.default.start(userCache, skillToDead).then((result) => {
