...
export default {
    ...
    welcomeUsers: (CMD: string | undefined, userInfo: UserInfo) =>{
        pvpUsers.add(userInfo.username)

        if (pvpUsers.size === 2) {
           return pvpBattle.pvpStart(CMD, userInfo)
        }
        let tempScript: string = '';
        const tempLine =
            '=======================================================================\n\n';
    ...
    // 이건 프론트단에서 하는게 나을려나 ?
    getUsers:(CMD: string | undefined, userInfo: UserInfo) =>{
    ...
    // 6명이 되면 게임시작
    pvpStart:(CMD: string | undefined, userInfo: UserInfo) =>{
    ...
    // 마을로 보내는 로직 구현필요.
    userLeave:(CMD: string | undefined, userInfo: UserInfo) =>{
        let tempScript: string = '';
